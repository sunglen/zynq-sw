...
extern ER		mget_lod(ID schedid, PRI tskpri, uint_t *p_load) throw();
extern ER		get_nth(PRI tskpri, uint_t nth, ID *p_tskid) throw();
extern ER		mget_nth(ID schedid, PRI tskpri, uint_t nth, ID *p_tskid) throw();
...
#define imact_tsk(tskid, prcid)				mact_tsk(tskid, prcid)
#define iloc_spn(spnid)						loc_spn(spnid)
#define iunl_spn(spnid)						unl_spn(spnid)
#define itry_spn(spnid)						try_spn(spnid)
#define imsta_alm(almid, almtim, prcid)		msta_alm(almid, almtim, prcid)
#define iget_pid(p_prcid)					get_pid(p_prcid)
...
#define TA_NATIVE		UINT_C(0x01)	/* エミュレーションによる実現を禁止 */
