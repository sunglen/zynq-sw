...
wai_flg(ID flgid, FLGPTN waiptn, MODE wfmode, FLGPTN *p_flgptn)
{
	...
	if (p_selftsk->raster) {
		...
		p_winfo_flg = (WINFO_FLG *)(&(p_selftsk->winfo_obj));
		p_winfo_flg->waiptn = waiptn;
		p_winfo_flg->wfmode = wfmode;
		wobj_make_wait(p_my_pcb, (WOBJCB *) p_flgcb, TS_WAITING_FLG, p_selftsk);
		release_glock();
		dispatch();
		ercd = p_selftsk->winfo.wercd;
		if (ercd == E_OK) {
			*p_flgptn = p_winfo_flg->waiptn;
		}
		goto unlock_and_exit;
	}
	release_glock();
...
twai_flg(ID flgid, FLGPTN waiptn, MODE wfmode, FLGPTN *p_flgptn, TMO tmout)
{
	...
	if (p_selftsk->raster) {
		...
		p_winfo_flg->waiptn = waiptn;
		p_winfo_flg->wfmode = wfmode;
		wobj_make_wait_tmout(p_my_pcb, (WOBJCB *) p_flgcb, TS_WAITING_FLG,
															p_selftsk, tmout);
		release_glock();
		dispatch();
		ercd = p_selftsk->winfo.wercd;
